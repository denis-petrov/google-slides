...
export default function SlideMenu() {
    ...
    let slides = editor.presentation.slides.map(item => {
        ...
        return <div className='slide' data-is-checked={editor.selectionSlidesId.includes(item.id)} key={item.id}
            ...
            <span className='item-position' style={{userSelect: 'none'}}>{editor.presentation.slides.indexOf(item) + 1}</span>
            <Card className={'mb-3 slides-menu-item'}>
                <svg className='slides-menu-item-svg' id={item.id} style={{background: `0 0 / cover ${getSlideBackgroundById(item.id)}`}}>
                    {elements}
                </svg>
            </Card>
        </div>
